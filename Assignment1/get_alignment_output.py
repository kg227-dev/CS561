...
def sequence_alignment(linex, liney, f, score=0, affine=0):
    ...
    print("Indels: " + "number="+str(indels)+ ", mean length=" + str(round(get_mean_indel_length(out_line),1)), file=f)
...
def output(file1, file2, save_to_file, matrix, is_affine=0):
    ...
    for gene in distant:
        ...
        if is_affine:
            alignment_1, alignment_2, affine_align_score = affine(x, y, score_matrix)
        else:
        ...
        sequence_alignment(alignment_1, alignment_2, file_output, affine_align_score, 1)
