...
def affine(sequence1, sequence2, scoring_matrix):
    ...
    # This is the main "scorekeeper", tracks only match/mismatch
    M = np.zeros((m+1, n+1))
    # Tracks gap creation/elongation in sequence 1
    seq1_GAP = np.zeros((m+1, n+1))
    # Tracks gap creation/elongation in sequence 2
    seq2_GAP = np.zeros((m+1, n+1))
    ...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            ...
            match_score = M[i - 1][j - 1] + \
                scoring_matrix[sequence1[i - 1] + sequence2[j - 1]]
            ...
            seq1_GAP[i][j] = max(
                seq1_GAP[i - 1][j] + gap_extend, M[i - 1][j] + gap_open + gap_extend)

            # Matrix tracking best alignment of gap (creating one or extending one) in sequence 2
            seq2_GAP[i][j] = max(
                seq2_GAP[i][j - 1] + gap_extend, M[i][j - 1] + gap_open + gap_extend)
